...
class RegForm(forms.Form):
    username = forms.CharField(label='用户名', max_length=20, min_length=5,
                               widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '请输入5~20位的用户名'}))
    ...
    def clean(self):
        ...
        email_1 = self.cleaned_data.get('email', '')
        if not (code != '' and code == verification_code and email != '' and email_1 != '' and email == email_1):
...
class ChangeNicknameForm(forms.Form):
    ...
    def clean_nickname_new(self):
        ...
        if Profile.objects.filter(nickname=nickname_new).exists():
...
class ChangePasswordForm(forms.Form):
    old_password = forms.CharField(label='旧密码', min_length=8,
                               widget=forms.PasswordInput(attrs={'class': 'form-control', 'placeholder': '请输入旧密码'}))
    new_password = forms.CharField(label='新密码', min_length=8,
                               widget=forms.PasswordInput(attrs={'class': 'form-control', 'placeholder': '请输入新密码'}))

    new_password_again = forms.CharField(label='再输入一次新密码', min_length=8,
                                     widget=forms.PasswordInput(
...
class BindQQForm(forms.Form):
    username_or_email = forms.CharField(label='用户名或邮箱',
                                        widget=forms.TextInput(
                                            attrs={'class': 'form-control', 'placeholder': '请输入用户名或密码'}))
    password = forms.CharField(label='密码',
                               widget=forms.PasswordInput(attrs={'class': 'form-control', 'placeholder': '请输入密码'}))
